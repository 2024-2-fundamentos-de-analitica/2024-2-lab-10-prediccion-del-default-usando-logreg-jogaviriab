...
def read_csv_from_zip(zip_path):
    ...
    with zipfile.ZipFile(zip_path, "r") as z:
        file_name = z.namelist()[0]  # Obtiene el primer archivo dentro del ZIP
...
def clean_data(data):
    ...
    data.drop(columns=["ID"], inplace=True)  # Eliminar columna ID
    data.dropna(inplace=True)  # Eliminar valores nulos
    
    # Filtrar datos incorrectos en EDUCATION y MARRIAGE
    data = data[(data["EDUCATION"] != 0) & (data["MARRIAGE"] != 0)]
    
    # Agrupar valores mayores a 4 en EDUCATION dentro de la categoría 4
    data["EDUCATION"] = data["EDUCATION"].apply(lambda x: 4 if x > 4 else x)
    
    return data

def classifier_model():
    """Crea un pipeline para el modelo de clasificación."""
...
def optimize_hyperparameters(model, x_train, y_train):
    """Optimiza los hiperparámetros del modelo mediante GridSearchCV."""
    param_grid = {
...
def save_model(model):
    """Guarda el modelo optimizado en un archivo comprimido con gzip."""
...
def confusion_matrix_metrics(model, x_train, y_train, x_test, y_test):
    """Calcula las matrices de confusión para entrenamiento y prueba."""
    y_train_pred = model.predict(x_train)
    y_test_pred = model.predict(x_test)
    ...
    cm_metrics_train = {"dataset": "train", "confusion_matrix": cm_train.tolist()}
    cm_metrics_test = {"dataset": "test", "confusion_matrix": cm_test.tolist()}
